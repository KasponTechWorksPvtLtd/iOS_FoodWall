//
//  KYButtonCells.swift
//  KYButton
//
//  Created by cuber7788 on 2016/10/7.
//  Copyright © 2016年 Lawliet. All rights reserved.
//

import UIKit

class KYButtonCells: UIView {

    /*
    // Only override draw() if you perform custom drawing.
    // An empty implementation adversely affects performance during animation.
    override func draw(_ rect: CGRect) {
        // Drawing code
    }
    */

    
    weak var actionButton:KYButton?
    fileprivate let circleLayer = CAShapeLayer()
    fileprivate let tintLayer = CAShapeLayer()
    let circlePath = UIBezierPath(roundedRect: CGRect(x: 6, y: 0, width: 60, height: 60), cornerRadius: 20)
    
    
    var actionCloure : ((KYButtonCells) -> Void)?
    
    
    var originColor : UIColor?
    open var buttonItemColor : UIColor = UIColor.white {
        didSet{
            circleLayer.fillColor = buttonItemColor.cgColor
            originColor? = buttonItemColor
        }
    }
    
    open var titleColor : UIColor = UIColor.red{
        didSet{
            titleLabel.textColor = titleColor
        }
    }
    
    var _titleLabel:UILabel? = nil
    open var titleLabel :UILabel {
        get{
            if _titleLabel == nil{
                _titleLabel = UILabel()
                _titleLabel?.textColor = titleColor
                addSubview(_titleLabel!)
            }
            return _titleLabel!
        }
    }
    
    var  i = 0
    open var title: String? = nil {
        didSet {
            
           
            if i == 0{
            
            titleLabel.font = UIFont(name: "Roboto-Medium", size: 6)
            titleLabel.text = title
            titleLabel.sizeToFit()

            titleLabel.frame.origin.x = -210.0
            titleLabel.frame.origin.y =  -15-titleLabel.frame.size.height/2
            titleLabel.frame.size.height = 60.0
            titleLabel.frame.size.width = 200.0
            titleLabel.layer.cornerRadius = 10.0
            titleLabel.layer.masksToBounds = true
            titleLabel.numberOfLines = 2
            titleLabel.adjustsFontSizeToFitWidth = true
            titleLabel.applyGradienttitleLabel()
                i = 1
                let label = UILabel(frame: CGRect(x: 210-titleLabel.frame.size.width, y:25-titleLabel.frame.size.height/2, width: 200, height: 60.0))
               // label.center = CGPoint(x: 50-titleLabel.frame.size.width, y: -15-titleLabel.frame.size.height/2)
                label.textAlignment = .left
                label.numberOfLines = 2
                label.text = title
                label.textColor = UIColor.white
                label.layer.cornerRadius = 10.0
                label.layer.masksToBounds = true
                titleLabel.font = UIFont(name: "Roboto-Medium", size: 5)
                self.titleLabel.addSubview(label)
                        //titleLabel.backgroundColor = UIColor.brown
           // titleLabel.frame.size.height = titleLabel.frame.origin.x/5
           // print(titleLabel.frame.size.height)
            
            }
            
            else
            {
                titleLabel.font = UIFont(name: "Roboto-Medium", size: 6)
                titleLabel.text = title
                titleLabel.sizeToFit()
                
                titleLabel.frame.origin.x = 30-titleLabel.frame.size.width
                titleLabel.frame.origin.y =  -15-titleLabel.frame.size.height/2
                titleLabel.frame.size.height = 60.0
                titleLabel.frame.size.width = 200.0
                titleLabel.applyGradienttitleLabel()
                
                
            }
            
        }
    }
    
    var _iconImageView: UIImageView? = nil
    open var iconImageView: UIImageView {
        get {
            if _iconImageView == nil {
                _iconImageView = UIImageView(frame: CGRect(x:25, y: 25, width: 30, height: 30))
                _iconImageView?.contentMode = UIViewContentMode.center
                addSubview(_iconImageView!)
            }
            return _iconImageView!
        }
    }
    
    open var icon: UIImage? = nil {
        didSet {
            iconImageView.image = icon
        }
    }
    
    override var frame: CGRect{
        didSet{
            _iconImageView?.center = CGPoint(x: self.frame.width/2, y: self.frame.height/3.2+6)
        }
    }
    
    
    public init(){
        super.init(frame:CGRect.zero)
        self.creatCircle()
        self.createShadow()
        self.createLabelShadow()
    }
    
    override init(frame: CGRect) {
        super.init(frame: frame)
    }
    
    required init?(coder aDecoder: NSCoder) {
        fatalError("init(coder:) has not been implemented")
    }
    
    
    fileprivate func creatCircle() {
        
        circleLayer.path = circlePath.cgPath
        circleLayer.fillColor = buttonItemColor.cgColor
        originColor = buttonItemColor
        layer.addSublayer(circleLayer)
        
    }
    
    fileprivate func creatLayer(){
        
        tintLayer.path = circlePath.cgPath
        tintLayer.fillColor = UIColor.orange.withAlphaComponent(0.5).cgColor // white
        layer.addSublayer(tintLayer)
    }
    
    fileprivate func createShadow(){
        circleLayer.shadowOffset = CGSize(width: 0, height: 0)
        circleLayer.shadowOpacity = 1
    }
    
    fileprivate func createLabelShadow(){
        titleLabel.layer.shadowOffset = CGSize(width: 1, height: 1)
        titleLabel.layer.shadowRadius = 2
        titleLabel.layer.shadowColor = UIColor.black.cgColor
        titleLabel.layer.shadowOpacity = 0.5
    }
    
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        self.creatLayer()
    }
    
    
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        tintLayer.removeFromSuperlayer()
        if let touch = touches.first{
           if touch.tapCount == 1 {
            actionCloure?(self)
            actionButton!.closeButton()
            }
        }
        circleLayer.fillColor = originColor?.cgColor
        
    }
    
}

extension UIView {
    func applyGradienttitleLabel() {
        let gradient = CAGradientLayer()
        
        let yellowColor  = UIColor(red: 1.0, green: 0.7, blue: 0.2, alpha: 1.5).cgColor
        let orangeColor  = UIColor(red: 255/255, green: 128/255, blue:8/255, alpha: 1.5).cgColor
        
        
        
        
        gradient.colors = [orangeColor,yellowColor,yellowColor]   // your colors go here
        gradient.startPoint = CGPoint(x: 0.0, y: 1.0)
        gradient.endPoint = CGPoint(x: 1.0, y: 1.0)
        gradient.frame = self.bounds
        self.layer.insertSublayer(gradient, at: 0)
    }
}
