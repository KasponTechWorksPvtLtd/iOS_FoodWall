...
class KYButton: UIView{
    ...
    */
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        buttonLayer?.backgroundColor = UIColor.orange.cgColor
...
//    ----------------------------------------------slideUp---------------------------------------------------
    ...
    fileprivate func slideUpAnimation(isShow:Bool) {
        ...
        for (index,item) in items.enumerated() {
            ...
            item.frame = CGRect(x: self.frame.origin.x, y: self.frame.origin.y - self.frame.height * CGFloat(index), width: self.frame.width, height: self.frame.height + 40)
